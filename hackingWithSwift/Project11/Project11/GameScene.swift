...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var balls = 5 {
        didSet{
            if balls > 0 {
                ballLabel.text = "Balls left: \(balls)"
            }
            else {
                ballLabel.text = "Try again?"
            }
        }
    }
    ...
    override func didMove(to view: SKView) {
        ...
        ballLabel.text = "Balls left: \(balls)"
        ballLabel.position = CGPoint(x: 880, y: 600)
        addChild(ballLabel)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if objects.contains(editLabel){
            ...
            if editingMode {
                ...
                if balls > 0 {
                    let ball = SKSpriteNode(imageNamed: generateColor())
    ...
    func generateColor() -> String {
        return ["ballRed","ballBlue","ballCyan","ballGreen","ballGrey","ballPurple","ballYellow"].randomElement()!
    }
