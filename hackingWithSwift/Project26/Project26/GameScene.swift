...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func playerColided(with node: SKNode){
        ...
        if node.name == "teleport"{
            if node.position != lastTp {
                lastTp = CGPoint(x: node.position.x, y: node.position.y)
                for teleport in teleports {
                    if teleport.position != lastTp {
                        outPoint = CGPoint(x: teleport.position.x, y: teleport.position.y)
//                        let move = SKAction.move(to: teleport.position, duration: 0.25)
//                        let scale = SKAction.scaleX(to: 0.0001, duration: 0.25)
//                        let remove = SKAction.removeFromParent()
//                        let sequence = SKAction.sequence([move, scale, remove])
//                        player.run(sequence){ [weak self] in
//                            self?.recreatePlayer(tp: teleport)
//                        }
                        if outPoint != lastTp {
                            player.removeFromParent()
                            recreatePlayer(tp: teleport)
                        }
                    }
                }
            }
            ...
            gameOverLabel.color = .gray
            addChild(gameOverLabel)
            return
        }
    }
    
    func recreatePlayer(tp: SKSpriteNode){
        player = SKSpriteNode(imageNamed: "player")
        player.position = CGPoint(x: tp.position.x + 70, y: tp.position.y)
        player.zPosition = 1
